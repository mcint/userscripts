...
$(() => {
	...
	function popupFilterLastModified(data, download) {
		...
		if (lastmod && getValueOf('popupLastModified')) {
			// LOCAL MOD (mcint/userscripts): show an absolute last-modified
			// timestamp (local time preferred) plus a compact age like 2d / 5h /
			// 3M / 5y, instead of upstream's relative "%s old". The UTC form is in
			// the title= tooltip; the (l) zone marker is local.
			// TODO: make the (l) marker a click-toggle to UTC (needs a pinned popup
			// to hold the click — design before building).
			// Upstream original was:
			//   return tprintf('%s old', [formatAge(age)]).replace(/ /g, '&nbsp;');
			const utc = lastmod.toISOString().slice(0, 16).replace('T', ' ') + 'Z';
			return '<span title="' + utc + ' (UTC)">' + localTimestamp(lastmod) +
				'&nbsp;(l)</span>&nbsp;(' + compactAge(age) + ')';
		}
		return '';
	}

	// LOCAL MOD (mcint/userscripts): compact single-unit age — digits + unit letter
	// (y, M, w, d, h, m, s; capital M = months, lowercase m = minutes). Returns the
	// largest unit that fits, e.g. 2d / 5h / 3M / 5y.
	function compactAge(age) {
		const s = 1000, m = 60 * s, h = 60 * m, d = 24 * h, w = 7 * d,
			mo = 30.44 * d, y = 365.25 * d;
		const a = Math.abs(0 + age);
		const units = [[y, 'y'], [mo, 'M'], [w, 'w'], [d, 'd'], [h, 'h'], [m, 'm'], [s, 's']];
		for (let i = 0; i < units.length; ++i) {
			if (a >= units[i][0]) { return Math.floor(a / units[i][0]) + units[i][1]; }
		}
		return '0s';
	}

	// LOCAL MOD (mcint/userscripts): local-time "YYYY-MM-DD HH:MM" (nbsp-joined).
